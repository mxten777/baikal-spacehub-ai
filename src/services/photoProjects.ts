...
const DEFAULT_STAGES: ProjectStage[] = ['source', 'selected', 'edited', 'web', 'pdf']
...
export async function getPhotoProjectById(id: string): Promise<PhotoProject | null> {
  const { data, error } = await supabase
    .from('photo_projects')
    .select('*')
    .eq('id', id)
...
export async function getPhotoProjectBySlug(slug: string): Promise<PhotoProject | null> {
  const { data, error } = await supabase
    .from('photo_projects')
    .select('*')
    .eq('slug', slug)
...
export async function createPhotoProject(
  ...
  const stages: ProjectStage[] = input.stages && input.stages.length > 0
    ? input.stages
    : DEFAULT_STAGES
...
export async function updatePhotoProject(
  ...
  if (input.categories !== undefined) updates.categories = [...new Set(input.categories)]
