...
const SPACE_IMAGES: Record<string, string> = {
  ...
  studio: 'https://images.unsplash.com/photo-1497366811353-6870744d04b2?w=900&q=80',
  storage: 'https://images.unsplash.com/photo-1600566752355-35792bedcfea?w=900&q=80',
  hall: 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=900&q=80',
  other: 'https://images.unsplash.com/photo-1541516160071-4bb0c5af65ba?w=900&q=80',
}

const FALLBACK_SPACES = [
  {
    id: '1', slug: 'cafe', name: '카페', name_en: 'Cafe', category: 'cafe' as SpaceCategory,
    short_description: '따뜻한 분위기의 문화 카페 공간. 소규모 모임, 낭독회, 소셜 이벤트에 적합합니다.',
    capacity: 50, size_sqm: 85, is_available: true,
    features: ['자연 채광', '커피 바', '빔 프로젝터', '음향 시스템'],
    cover_image: SPACE_IMAGES.cafe,
  },
  {
    id: '2', slug: 'garden', name: '가든', name_en: 'Garden', category: 'garden' as SpaceCategory,
    short_description: '자연과 어우러진 야외 정원 공간. 웨딩, 파티, 야외 공연에 최적입니다.',
    capacity: 120, size_sqm: 200, is_available: true,
    features: ['야외 테라스', '조명 시설', '음향 시스템', '주차 가능'],
    cover_image: SPACE_IMAGES.garden,
  },
  {
    id: '3', slug: 'studio', name: '스튜디오', name_en: 'Studio', category: 'studio' as SpaceCategory,
    short_description: '전문 촬영 및 공연을 위한 화이트 스튜디오. 광고, 화보, 소규모 공연에 적합.',
    capacity: 30, size_sqm: 65, is_available: true,
    features: ['사이클로라마', '전문 조명', '분장실', '드레스룸'],
    cover_image: SPACE_IMAGES.studio,
  },
  {
    id: '4', slug: 'storage', name: '스토리지', name_en: 'Storage Hall', category: 'storage' as SpaceCategory,
    short_description: '대형 행사와 전시를 위한 다목적 홀. 오픈 플로어 플랜으로 자유로운 공간 활용 가능.',
    capacity: 150, size_sqm: 280, is_available: true,
    features: ['오픈 플로어', '높은 천장', '전시 벽면', '케이터링 준비실'],
    cover_image: SPACE_IMAGES.storage,
  },
]

export default function SpacesPage() {
  const [activeCategory, setActiveCategory] = useState<string>('all')
  const { data: spaces, isLoading } = useSpaces(activeCategory !== 'all' ? { category: activeCategory } : undefined)
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="더릿의 다양한 공간을 살펴보세요. 카페, 가든, 스튜디오, 스토리지 — 모든 문화 활동을 위한 프리미엄 공간 대관." />
      </Helmet>
        ...
        <div className="container-wide">
          <AnimatedSection animation="fade-up">
            ...
            <p className="font-sans text-base text-white/60 max-w-xl">
              더릿의 각 공간은 독자적인 분위기와 기능을 갖춘 프리미엄 문화 공간입니다.
              카페, 가든, 스튜디오, 스토리지 — 당신의 이야기에 맞는 공간을 선택하세요.
            </p>
          </AnimatedSection>
        </div>
      </section>
                  ...
                  <Link to={`/spaces/${space.slug}`} className="group block">
                    ...
                    <div className="pt-7 pb-2">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <p className="font-sans text-[10px] tracking-[0.2em] uppercase text-brand-subtle mb-1.5">{space.name_en}</p>
                          <h2 className="font-display text-[1.75rem] font-light text-brand-black" style={{ letterSpacing: '-0.02em' }}>
                            {space.name}
                          </h2>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-5">
                        {(space.features ?? []).slice(0, 4).map((f) => (
                          <span key={f} className="tag-outline">{f}</span>
                        ))}
                      </div>
                      <div className="flex items-center gap-2 font-sans text-[11px] tracking-[0.16em] uppercase text-brand-black group-hover:text-brand-accent transition-colors duration-300">
                        공간 상세 보기 <ArrowRight size={13} className="group-hover:translate-x-1 transition-transform duration-300" />
                      </div>
                    </div>
                  </Link>
                </AnimatedSection>
