    ...
    <>
      ...
      <article className="section-padding">
        <div className="container-narrow">
          <Link
            to="/blog"
            className="inline-flex items-center gap-2 text-sm text-brand-muted hover:text-brand-black transition-colors mb-10"
          >
          ...
          <AnimatedSection animation="fade-up">
            {displayPost.category && (
              <p
                ...
                style={{ color: displayPost.category.color }}
              >
            ...
            <div className="flex items-center gap-4 mb-10 pb-8 border-b border-brand-border">
              {displayPost.author?.full_name && (
                <span className="font-sans text-sm text-brand-muted">
                  by {displayPost.author.full_name}
                </span>
              )}
              {displayPost.published_at && (
                <span className="font-sans text-sm text-brand-muted">
                  {format(
                    new Date(displayPost.published_at),
                    "yyyy년 M월 d일",
                    { locale: ko },
                  )}
                </span>
            ...
            <div className="max-w-none">
              <ReactMarkdown
                ...
                components={{
                  h1: ({ children }) => (
                    <h1 className="font-display text-2xl font-light text-brand-black mt-10 mb-4 leading-tight">
                      {children}
                    </h1>
                  ),
                  h2: ({ children }) => (
                    <h2 className="font-display text-xl font-light text-brand-black mt-8 mb-3 leading-tight">
                      {children}
                    </h2>
                  ),
                  h3: ({ children }) => (
                    <h3 className="font-display text-lg font-light text-brand-black mt-6 mb-2">
                      {children}
                    </h3>
                  ),
                  p: ({ children }) => (
                  ...
                  ul: ({ children }) => (
                    <ul className="list-disc pl-6 mb-5 space-y-1.5 font-sans text-[15px] text-brand-muted">
                      {children}
                    </ul>
                  ),
                  ol: ({ children }) => (
                    <ol className="list-decimal pl-6 mb-5 space-y-1.5 font-sans text-[15px] text-brand-muted">
                      {children}
                    </ol>
                  ),
                  li: ({ children }) => (
                  ...
                  blockquote: ({ children }) => (
                    <blockquote className="border-l-[3px] border-brand-accent pl-5 my-6 text-brand-muted italic">
                      {children}
                    </blockquote>
                  ),
                  pre: ({ children }) => (
                    <pre className="bg-brand-cream p-4 rounded overflow-x-auto mb-5 text-sm font-mono leading-relaxed">
                      {children}
                    </pre>
                  ),
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  code: ({ children, className }: any) => {
                    ...
                    return (
                      <code className="bg-brand-cream px-1.5 py-0.5 text-sm font-mono rounded text-brand-black">
                        {children}
                      </code>
                    );
                  },
                  ...
                  th: ({ children }) => (
                    <th className="text-left border-b border-brand-border pb-2 pr-4 font-semibold text-brand-black">
                      {children}
                    </th>
                  ),
                  td: ({ children }) => (
                    <td className="border-b border-brand-line py-2 pr-4 text-brand-muted">
                      {children}
                    </td>
                  ),
                }}
