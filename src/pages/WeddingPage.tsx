...
// ─── Wedding Story fallbacks ────────────────────────────────────────────────
const STORY_TITLE_FALLBACK = "틀에 박힌 웨딩이 아니라,\n두 사람만의 장면을";
const STORY_PARAGRAPHS_FALLBACK = [
  "THE LIT는 단순한 웨딩홀이 아닙니다. 문화가 숨 쉬는 복합공간이 웨딩의 무대가 될 때, 그 하루는 하나의 작품이 됩니다.",
...
export default function WeddingPage() {
  ...
  // ── Wedding Story ─────────────────────────────────────────────────────────
  const storyTitle = aboutContent?.wedding_story_title ?? STORY_TITLE_FALLBACK;
  const storyParagraphs: string[] =
    (aboutContent?.wedding_story_paragraphs?.length ?? 0) > 0
      ? (aboutContent!.wedding_story_paragraphs as string[])
      : STORY_PARAGRAPHS_FALLBACK;
  const [storyLine1, storyLine2] = storyTitle.split("\n");
