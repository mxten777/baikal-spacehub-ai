...
function ArchiveItemForm({ initialData, onClose, onSuccess }: { initialData?: ArchiveItem; onClose: () => void; onSuccess: () => void }) {
  const [saving, setSaving] = useState(false)
  const { register, handleSubmit, formState: { errors } } = useForm<ArchiveFormData>({
    ...
    defaultValues: initialData
      ...
      : { title: '', slug: '', description: '', category: '', date: '', images: '', is_featured: false },
  })
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="font-display text-lg font-light">{initialData ? '아카이브 편집' : '아카이브 추가'}</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit as any /* eslint-disable-line @typescript-eslint/no-explicit-any */)} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input {...register('category')} placeholder="전시, 공연, 이벤트..." className="w-full border border-gray-200 px-3 py-2 text-sm font-sans focus:outline-none focus:border-brand-black" />
              {errors.category && <p className="text-red-500 text-xs mt-1">{errors.category.message}</p>}
            </div>
            <div>
              <label className="block text-xs font-sans text-gray-600 tracking-wider uppercase mb-1">날짜</label>
              <input type="date" {...register('date')} className="w-full border border-gray-200 px-3 py-2 text-sm font-sans focus:outline-none focus:border-brand-black" />
            </div>
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs text-gray-600 tracking-wider uppercase mb-1">이미지 URL (줄바꿈으로 구분)</label>
            <textarea {...register('images')} rows={4} placeholder="https://example.com/image1.jpg&#10;https://example.com/image2.jpg" className="w-full border border-gray-200 px-3 py-2 text-sm font-mono focus:outline-none focus:border-brand-black resize-none" />
          </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display text-2xl font-light text-brand-black">Archive</h1>
                ...
                archives.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <p className="font-sans text-sm font-medium text-brand-black">{item.title}</p>
                      {item.is_featured && (
                        <span className="text-[10px] font-sans tracking-widest uppercase text-brand-accent">Featured</span>
                      )}
                    </td>
                    <td className="px-6 py-4 hidden md:table-cell">
                      <span className="text-xs font-sans text-gray-600">{item.category}</span>
                    </td>
