...
function StatCard({ icon: Icon, label, value, href, color = 'bg-brand-black' }: StatCardProps) {
  return (
    <Link to={href} className="group block bg-white rounded-none border border-gray-200 p-6 hover:border-brand-black transition-colors">
      <div className="flex items-center justify-between mb-4">
        <div className={`w-10 h-10 ${color} flex items-center justify-center`}>
          <Icon size={18} className="text-white" />
        </div>
        <TrendingUp size={16} className="text-gray-400 group-hover:text-brand-black transition-colors" />
      </div>
      <p className="font-sans text-3xl font-light text-brand-black mb-1">{value}</p>
...
export default function AdminDashboard() {
  ...
  const stats = [
    ...
    { icon: Calendar, label: 'Programs', value: programs?.length ?? 0, href: '/admin/programs', color: 'bg-purple-600' },
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-display text-2xl font-light text-brand-black">Dashboard</h1>
        <p className="font-sans text-sm text-gray-500 mt-1">The Lit 관리자 대시보드</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {[
              { label: 'New Program', href: '/admin/programs/new', icon: Calendar },
        ...
        <div className="bg-white border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/inquiries" className="text-xs text-brand-accent hover:underline">View all</Link>
          </div>
            ...
            <div className="space-y-3">
              {inquiries.slice(0, 5).map((inq) => (
                ...
                >
                  ...
                  <p className="font-sans text-xs text-gray-500 mt-0.5 line-clamp-1">{inq.subject}</p>
                </Link>
              ))}
            </div>
          ) : (
            <p className="font-sans text-sm text-gray-400">대기 중인 문의가 없습니다</p>
          )}
        </div>
      </div>
    </div>
