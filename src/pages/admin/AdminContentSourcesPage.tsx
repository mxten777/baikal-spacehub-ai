      ...
      <div className="bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit as any)} className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-6 pt-2">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" {...register('auto_publish')} className="w-4 h-4" />
              <span className="text-sm font-sans text-gray-700">수집 즉시 자동 공개</span>
            </label>
          </div>
...
export default function AdminContentSourcesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('이 소스를 삭제하시겠습니까? 수집된 콘텐츠는 유지됩니다.')) return
  ...
  return (
    <div>
      ...
      {fetchResult && (
        <div className={`mb-4 p-3 text-sm font-sans border ${
          fetchResult.startsWith('✓') ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'
        }`}>
          {fetchResult}
          <button onClick={() => setFetchResult(null)} className="ml-3 opacity-60 hover:opacity-100">✕</button>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400 border border-dashed border-gray-200">
          ...
          <p className="font-sans text-xs mt-1">
            위 "새 소스 추가" 버튼으로 RSS, YouTube, Instagram, X 소스를 등록하세요.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 truncate">
                    {source.rss_url ?? source.channel_id ?? source.account_handle ?? source.source_url ?? '—'}
                  </p>
                  <p className="text-xs text-gray-300 mt-0.5">
                    수집 주기: {source.fetch_interval_minutes}분 ·{' '}
                    마지막 수집:{' '}
