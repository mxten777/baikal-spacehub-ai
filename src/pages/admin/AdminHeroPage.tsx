        ...
        <form onSubmit={handleSubmit(onSubmit as Parameters<typeof handleSubmit>[0])} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-xs font-sans text-gray-600 tracking-wider uppercase mb-1">
              타이틀 * <span className="text-gray-400 normal-case tracking-normal">(줄바꿈: \n 입력)</span>
            </label>
            <input
              {...register('title')}
          ...
          <p className="text-[11px] font-sans text-gray-400 -mt-2">
            모바일 이미지를 설정하지 않으면 데스크톱 이미지가 모바일에도 표시됩니다.
          </p>

          {/* 버튼 1 */}
          <div>
            <p className="text-xs font-sans text-gray-600 tracking-wider uppercase mb-2">첫 번째 버튼</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-sans text-gray-500 mb-1">버튼 텍스트</label>
                <input
          ...
          {/* 버튼 2 */}
          <div>
            <p className="text-xs font-sans text-gray-600 tracking-wider uppercase mb-2">두 번째 버튼</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-sans text-gray-500 mb-1">버튼 텍스트</label>
                <input
...
export default function AdminHeroPage() {
  ...
  const nextOrder = slides ? Math.max(0, ...slides.map((s) => s.display_order)) + 1 : 1
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="font-sans text-sm text-gray-500 mt-1">홈페이지 메인 Hero 슬라이드 관리</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 border border-dashed border-gray-200">
          <p className="font-sans text-sm text-gray-400">등록된 슬라이드가 없습니다.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {slides.map((slide, i) => {
            const { label: statusLabel, color: statusColor } = getSlideStatus(slide)
            const isFirst = i === 0
              ...
              >
                ...
                <div className="w-20 h-14 bg-gray-100 shrink-0 overflow-hidden">
                  {slide.desktop_image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="text-[10px] font-sans text-gray-300">No image</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className={`inline-block px-2 py-0.5 text-[10px] font-sans tracking-widest uppercase ${statusColor}`}>
                      {statusLabel}
                    </span>
                  ...
                  {slide.subtitle && (
                    <p className="font-sans text-xs text-gray-500 truncate">{slide.subtitle}</p>
                  )}
                  {(slide.primary_button_text || slide.secondary_button_text) && (
                    <p className="font-sans text-[11px] text-gray-400 mt-1">
                      버튼: {[slide.primary_button_text, slide.secondary_button_text].filter(Boolean).join(' · ')}
                    </p>
                  )}
                  {(slide.publish_start_at || slide.publish_end_at) && (
                    <p className="font-sans text-[11px] text-gray-400">
                      기간: {slide.publish_start_at ? isoToLocal(slide.publish_start_at) : '∞'} ~{' '}
                      {slide.publish_end_at ? isoToLocal(slide.publish_end_at) : '∞'}
                    </p>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  {/* Edit */}
                  <button
                    onClick={() => { setEditingSlide(slide); setFormOpen(true) }}
                    title="편집"
                    className="w-7 h-7 flex items-center justify-center text-gray-400 hover:text-brand-black transition-colors"
                  >
                    <Pencil size={14} />
