...
import { Calendar, User, Phone, Mail, Building2, ChevronDown, ExternalLink } from 'lucide-react'
...
const STATUS_CONFIG: Record<ReservationStatus, { label: string; color: string; bg: string }> = {
  ...
  consulting:  { label: '상담중',   color: 'text-amber-700',  bg: 'bg-amber-50 border-amber-200' },
  quote_sent:  { label: '견적 발송', color: 'text-purple-700', bg: 'bg-purple-50 border-purple-200' },
  confirmed:   { label: '예약 확정', color: 'text-green-700',  bg: 'bg-green-50 border-green-200' },
  completed:   { label: '행사 완료', color: 'text-brand-muted', bg: 'bg-brand-cream border-brand-border' },
  cancelled:   { label: '취소',     color: 'text-rose-700',   bg: 'bg-rose-50 border-rose-200' },
}

const STATUS_ORDER: ReservationStatus[] = ['new', 'consulting', 'quote_sent', 'confirmed', 'completed', 'cancelled']

const EVENT_TYPE_LABELS: Record<string, string> = {
  exhibition: '전시', performance: '공연', workshop: '워크숍',
  brand_event: '브랜드 행사', corporate: '기업행사', shoot: '촬영',
  wedding: '웨딩', gathering: '모임', consultation: '공간 상담',
}
...
function StatusBadge({ status }: { status: ReservationStatus }) {
  ...
  return (
    <span className={`inline-flex px-2 py-0.5 font-sans text-[10px] font-medium border ${cfg.bg} ${cfg.color}`}>
      {cfg.label}
    </span>
  )
}
...
function ReservationRow({ r, onStatusChange }: {
  r: Reservation
      ...
      >
        ...
        <div className="flex-1 grid grid-cols-5 gap-4 items-center min-w-0">
          <div className="col-span-2 min-w-0">
            <p className="font-sans text-sm font-medium text-brand-black truncate">{r.name}</p>
            <p className="font-sans text-xs text-brand-subtle truncate">{r.phone}</p>
          </div>
          <div>
        ...
        <div className="border-t border-brand-line bg-brand-ivory px-5 py-5">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="space-y-2">
              <p className="font-sans text-[9px] tracking-[0.2em] uppercase text-brand-subtle mb-3">연락처</p>
              <div className="flex items-center gap-2 text-xs text-brand-black">
                <User size={12} className="text-brand-muted shrink-0" /> {r.name}
              </div>
              <div className="flex items-center gap-2 text-xs text-brand-black">
              ...
              {r.company && (
                <div className="flex items-center gap-2 text-xs text-brand-black">
                  <Building2 size={12} className="text-brand-muted shrink-0" /> {r.company}
                </div>
              )}
            </div>
            ...
            {/* Event details */}
            <div className="space-y-2">
              <p className="font-sans text-[9px] tracking-[0.2em] uppercase text-brand-subtle mb-3">행사 정보</p>
              <div className="flex items-center justify-between text-xs">
                <span className="text-brand-muted">유형</span>
                <span className="text-brand-black">{EVENT_TYPE_LABELS[r.event_type]}</span>
              </div>
              {r.preferred_date && (
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-brand-black flex items-center gap-1">
                    <Calendar size={10} />
                    {r.preferred_date}
                    {r.date_flexible && <span className="text-brand-subtle">(유동)</span>}
                  </span>
                </div>
              )}
              {r.expected_attendees && (
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-brand-black">{r.expected_attendees}명</span>
                </div>
              )}
              {r.recommended_space && (
                <div className="flex items-center justify-between text-xs">
                  <span className="text-brand-muted">추천 공간</span>
                  <span className="text-brand-black">{r.recommended_space}</span>
                </div>
              )}
              {r.budget_range && (
                <div className="flex items-center justify-between text-xs">
            ...
            {/* Notes */}
            <div>
              <p className="font-sans text-[9px] tracking-[0.2em] uppercase text-brand-subtle mb-3">메모</p>
              {r.event_purpose && (
                <p className="font-sans text-xs text-brand-black mb-2 leading-relaxed">
                  <span className="text-brand-muted">목적: </span>{r.event_purpose}
                </p>
              )}
              {r.notes && (
                <p className="font-sans text-xs text-brand-black leading-relaxed">
                  <span className="text-brand-muted">요청: </span>{r.notes}
                </p>
              )}
              {!r.event_purpose && !r.notes && (
                <p className="font-sans text-xs text-brand-subtle italic">메모 없음</p>
              )}
            </div>
          </div>
              ...
              {statusOpen && (
                <div className="absolute left-0 top-full mt-1 w-36 bg-white border border-brand-border shadow-lg z-10">
                  {STATUS_ORDER.map(s => (
                    <button
                      key={s}
                      onClick={() => { onStatusChange(r.id, s); setStatusOpen(false) }}
...
export default function AdminReservationsPage() {
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="mb-8">
        <h1 className="font-display text-2xl font-light text-brand-black mb-1">예약 관리</h1>
        <p className="font-sans text-sm text-brand-muted">공간 예약 요청 및 CRM 상태 관리</p>
      </div>
          ...
          return (
            <button
              ...
              className={`p-3 text-left border transition-all ${
                statusFilter === s ? 'border-brand-black' : 'border-brand-line hover:border-brand-border'
              }`}
            >
              <p className="font-display text-2xl font-light text-brand-black">{stats[s] || 0}</p>
              <p className={`font-sans text-[9px] tracking-[0.15em] uppercase mt-0.5 ${cfg.color}`}>{cfg.label}</p>
            </button>
          )
        })}
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="font-sans text-xs text-brand-muted">
          {statusFilter === 'all' ? `전체 ${reservations.length}건` : `${STATUS_CONFIG[statusFilter].label} ${filtered.length}건`}
        </p>
        {statusFilter !== 'all' && (
          <button
      ...
      {/* Reservations list */}
      {isLoading ? (
        <div className="py-16 text-center font-sans text-sm text-brand-muted">불러오는 중...</div>
      ) : filtered.length === 0 ? (
        <div className="py-16 text-center border border-brand-line">
          <p className="font-display text-lg font-light text-brand-muted">예약 요청이 없습니다</p>
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map(r => (
