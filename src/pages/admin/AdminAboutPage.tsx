...
import type { AboutContent, AboutTimelineItem, AboutValueItem } from '../../types'
import { Check, Loader2, Plus, Trash2, GripVertical } from 'lucide-react'

type SectionKey = 'hero' | 'mission' | 'story' | 'timeline' | 'values' | 'cta'
...
export default function AdminAboutPage() {
  ...
  useEffect(() => {
    aboutService.get().then(setContent).finally(() => setLoading(false))
  }, [])

  const handleSave = useCallback(async (section: SectionKey, updates: Partial<Omit<AboutContent, 'id' | 'updated_at'>>) => {
    if (!content) return
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-display text-2xl font-light text-brand-black">About 페이지 관리</h1>
        <p className="font-sans text-sm text-gray-500 mt-1">About 페이지의 모든 콘텐츠를 편집합니다.</p>
      </div>
  ...
  return (
    <div className="bg-white border border-gray-200">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <h2 className="font-sans text-sm font-medium text-brand-black tracking-wide">{title}</h2>
        <button
          onClick={onSave}
          disabled={saving}
...
function Field({
  ...
  return (
    <div>
      <label className="block font-sans text-xs text-gray-500 mb-1.5">{label}</label>
      {multiline ? (
        <textarea
          value={value}
...
function HeroSection({
  ...
  const [heroTitleLine1, setHeroTitleLine1] = useState(content.hero_title_line1)
  const [heroTitleLine2, setHeroTitleLine2] = useState(content.hero_title_line2)
  ...
  return (
    <SectionCard title="Hero" saving={saving} saved={saved} onSave={() => onSave({
      hero_image_url: heroImageUrl,
      hero_eyebrow: heroEyebrow,
      hero_title_line1: heroTitleLine1,
      hero_title_line2: heroTitleLine2,
    })}>
      <Field label="배경 이미지 URL" value={heroImageUrl} onChange={setHeroImageUrl} placeholder="https://..." />
      <Field label="Eyebrow 텍스트" value={heroEyebrow} onChange={setHeroEyebrow} />
      <div className="grid grid-cols-2 gap-4">
        <Field label="제목 1행" value={heroTitleLine1} onChange={setHeroTitleLine1} placeholder="문화의 불꽃을" />
        <Field label="제목 2행 (이탤릭)" value={heroTitleLine2} onChange={setHeroTitleLine2} placeholder="켜는 공간" />
      </div>
    </SectionCard>
  )
}
...
function MissionSection({
  ...
  return (
    <SectionCard title="Our Mission" saving={saving} saved={saved} onSave={() => onSave({
      mission_quote: quote,
      mission_description: desc,
    })}>
      <Field label="미션 인용구" value={quote} onChange={setQuote} multiline rows={3} />
      <Field label="미션 설명" value={desc} onChange={setDesc} multiline rows={2} />
    </SectionCard>
  )
}
...
function StorySection({
  ...
  return (
    <SectionCard title="Our Story" saving={saving} saved={saved} onSave={() => onSave({
      ...
      <Field label="본문 단락 1" value={p1} onChange={setP1} multiline rows={3} />
      <Field label="본문 단락 2" value={p2} onChange={setP2} multiline rows={3} />
      <Field label="본문 단락 3" value={p3} onChange={setP3} multiline rows={3} />
    </SectionCard>
  )
}
...
function TimelineSection({
  ...
  return (
    <SectionCard title="타임라인" saving={saving} saved={saved} onSave={() => onSave({ timeline: items })}>
      <div className="space-y-3">
        {items.map((item, idx) => (
          <div key={idx} className="flex gap-3 p-4 border border-gray-100 bg-gray-50">
...
function ValuesSection({
  ...
  return (
    <SectionCard title="Core Values" saving={saving} saved={saved} onSave={() => onSave({
...
function CtaSection({
  ...
  return (
    <SectionCard title="CTA (하단 행동 유도)" saving={saving} saved={saved} onSave={() => onSave({
      cta_title: ctaTitle,
      cta_description: ctaDesc,
    })}>
      <Field label="CTA 제목" value={ctaTitle} onChange={setCtaTitle} />
      <Field label="CTA 설명" value={ctaDesc} onChange={setCtaDesc} multiline rows={2} />
    </SectionCard>
  )
}
